...
# Note that the "setxkbmap `cat ...`" command below is the same as what occurs during startup of the X session by lightdm as seen in /etc/lightdm/Xsession
/usr/bin/su kw -c "sleep 1; setxkbmap `cat /home/kw/.Xkbmap`" &
